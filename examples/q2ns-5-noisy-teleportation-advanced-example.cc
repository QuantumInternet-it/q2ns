 ...
 *---------------------------------------------------------------------------*/
/**
 * @file q2ns-5-noisy-teleportation-advanced-example.cc
 * @brief Teleportation experiment with UDP/TCP control transport and Bob-side memory noise
 *
 * Demonstrates:
 *   - quantum teleportation over a quantum link
 *   - classical correction delivery over UDP or TCP
 *   - receiver setup via PacketSinkHelper
 ...
 *
 * Example:
 *   ./ns3 run q2ns-5-noisy-teleportation-advanced-example -- --proto=udp --distanceKm=50 --TDepMs=5
 *--trials=20 --verbose=1
 *   ./ns3 run q2ns-5-noisy-teleportation-advanced-example -- --proto=tcp --distanceKm=50 --TDepMs=5
 *--trials=20 --verbose=1
 *---------------------------------------------------------------------------*/
...
void TryCorrections(Ptr<QNode> bob, NetController& net, BobInfo& info, Ptr<DepolarizingQMap> depol,
                    Ptr<UniformRandomVariable> u, std::shared_ptr<Qubit> qRef, Time tStart,
                    bool verbose) {
  ...
  const auto idealState = net.GetState(qRef);
...
static void OnControlRxTrace(Ptr<QNode> bob, BobInfo* bobInfo, NetController* net,
                             Ptr<DepolarizingQMap> depol, Ptr<UniformRandomVariable> u,
                             std::shared_ptr<Qubit> qRef, Time* tStart, bool verbose,
                             Ptr<const Packet> packet, const Address&) {
  uint8_t bytes[2] = {0, 0};
  ...
  if (verbose) {
    std::cout << "[RECV][classical][B] t=" << std::fixed << std::setprecision(6)
              << Simulator::Now().GetSeconds() << " s"
              << "  m1=" << bobInfo->m1 << "  m2=" << bobInfo->m2 << "\n";
  }

  TryCorrections(bob, *net, *bobInfo, depol, u, qRef, *tStart, verbose);
}

static void OnTcpConnectSuccess(std::shared_ptr<TcpSendState> tcpState, Ptr<Socket> s) {
  if (!tcpState->sent && tcpState->pendingPkt) {
    s->Send(tcpState->pendingPkt);
    tcpState->sent = true;

    if (tcpState->verbose) {
      std::cout << "[SEND][classical][tcp] A->B\n";
    }
...
static void OnTcpConnectFail(bool verbose, Ptr<Socket> s) {
  if (verbose) {
    std::cout << "[WARN] TCP connect failed\n";
  }
  s->Close();
}
...
TrialResult RunOnce(const std::string& proto, double distanceKm, double TDepMs, bool verbose) {
  ...
  auto qRef = ref->CreateQubit();
  ref->Apply(gates::H(), {qRef});
  ...
  // Receiver side: PacketSink handles the transport-specific listening/accept path.
  Address bindAddr = InetSocketAddress(Ipv4Address::GetAny(), port);
  PacketSinkHelper sinkHelper((proto == "udp") ? "ns3::UdpSocketFactory" : "ns3::TcpSocketFactory",
                              bindAddr);
  ApplicationContainer sinkApps = sinkHelper.Install(B);
  sinkApps.Start(Seconds(0.0));
  sinkApps.Stop(Seconds(5.0));

  Ptr<PacketSink> sink = DynamicCast<PacketSink>(sinkApps.Get(0));
  if (!sink) {
    NS_ABORT_MSG("Failed to install PacketSink on Bob");
  }

  // PacketSink trace hookup.
  // If your local ns-3 build wants RxWithAddresses instead, swap this accordingly.
  sink->TraceConnectWithoutContext("Rx", MakeBoundCallback(&OnControlRxTrace, B, &bobInfo, &net,
                                                           depol, u, qRef, &tStart, verbose));

  // Sender side:
  // UDP = connect/send/close immediately
  // TCP = connect asynchronously, send pending packet on connect success
  auto tcpState = std::make_shared<TcpSendState>();
  tcpState->verbose = verbose;

  // Bob's quantum receive callback
  B->SetRecvCallback(
      [B, &bobInfo, &net, depol, u, qRef, &tStart, verbose](std::shared_ptr<Qubit> q) {
        bobInfo.qRemote = q;
        ...
        TryCorrections(B, net, bobInfo, depol, u, qRef, tStart, verbose);
      });

  auto sendCtrlPacket = [&](Ptr<Packet> pkt) {
    InetSocketAddress dst(interfaces.GetAddress(1), port);

    if (proto == "udp") {
      Ptr<Socket> sock = Socket::CreateSocket(A, UdpSocketFactory::GetTypeId());
      if (sock->Connect(dst) == 0) {
        ...
        std::cout << "[WARN] UDP connect failed\n";
      }
      sock->Close();
      return;
    }

    Ptr<Socket> sock = Socket::CreateSocket(A, TcpSocketFactory::GetTypeId());
    tcpState->pendingPkt = pkt;
    tcpState->sent = false;

    sock->SetConnectCallback(MakeBoundCallback(&OnTcpConnectSuccess, tcpState),
                             MakeBoundCallback(&OnTcpConnectFail, verbose));

    int rc = sock->Connect(dst);
    if (rc != 0) {
      if (verbose) {
        std::cout << "[WARN] TCP connect() immediate failure\n";
      }
      sock->Close();
      tcpState->pendingPkt = nullptr;
    }
  };
  ...
  Simulator::Schedule(NanoSeconds(1), [&]() {
    ...
    sendCtrlPacket(Create<Packet>(bytes, 2));
  });
...
int main(int argc, char* argv[]) {
  ...
  CommandLine cmd;
  cmd.AddValue("proto", "udp | tcp", proto);
  ...
  if (proto != "udp" && proto != "tcp") {
    NS_ABORT_MSG("--proto must be udp or tcp");
  }
  ...
  for (uint32_t t = 0; t < trials; ++t) {
    ...
    TrialResult r = RunOnce(proto, distanceKm, TDepMs, verbose);
    fidelityStats.Add(r.fidelity);
    latencyStats.Add(r.completionMs);
  }

  std::cout << "\n[RESULT]"
            << " proto=" << proto << " distance_km=" << std::fixed << std::setprecision(3)
            << distanceKm << " TDep_ms=" << TDepMs << " trials=" << trials
