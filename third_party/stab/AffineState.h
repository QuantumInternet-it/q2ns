 ...
 * - Added DropLastQubit()
 * - Added TensorProduct(const AffineState&, const AffineState&)
 * - Updated optional qpp include path for local integration
...
public:
  ...
  void DropLastQubit();
  static AffineState TensorProduct(const AffineState& a, const AffineState& b);
...
using expr_t = typename std::decay<decltype(std::declval<Derived>().eval())>::type;
