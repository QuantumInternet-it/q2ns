 ...
 * - Updated optional qpp integration details
...
int AffineState::n() const {
  return n_;
}

int AffineState::phase() const {
  return phase_;
}

// Only return the top-left block, because rows/cols r+1, ..., n are not active
mat_u_t AffineState::Q() const {
  return Q_.block(0, 0, r_, r_);
}

// Similarly, only return the active portion of A
mat_u_t AffineState::A() const {
  return A_.block(0, 0, n_, r_);
}

vec_u_t AffineState::b() const {
  return b_;
}

std::vector<int> AffineState::pivots() const {
  return pivots_;
}

int AffineState::r() const {
  return r_;
}
...
qpp::ket AffineState::to_ket() const {
  ...
  if (n_ > MAX_QUBITS_STATE_VECTOR) {
    throw std::logic_error("Maximum number of qubits for state vector representation is " +
  ...
  for (unsigned k = 0; k < D_r; ++k) {
    ...
    auto basis_state_number = internal::multiidx2n(basis_state_x.data(), dims.size(), dims.data());

    // Compute the phase
    cplx phase = (2 * (x.transpose() * Q_.block(0, 0, r_, r_) * x)[0] + phase_) % 8;
    // Fill in corresponding element of psi
    psi(basis_state_number) += std::exp(1.0_i * qpp::pi * phase / static_cast<qpp::realT>(4.0));
  }
...
void AffineState::DropLastQubit() {
  if (n_ <= 0) {
    throw std::logic_error("DropLastQubit on empty state");
  }
  const int dropped = n_ - 1;

  // Ensure no active pivot points at the row we're about to drop.
  for (int c = 0; c < r_; ++c) {
    if (pivots_[c] == dropped) {
      // Try to reselect a different principal row; if impossible, eliminate the column.
      if (!ReselectPrincipalRow(dropped, c)) {
        ZeroColumnElim(c);
      }
    }
  }

  // Clear the dropped row in A_ and b_.
  for (int c = 0; c < r_; ++c) {
    A_(dropped, c) = 0;
  }
  b_(dropped) = 0;

  // Reduce qubit count. We keep allocated matrix sizes; loops use n_.
  --n_;
}

AffineState AffineState::TensorProduct(const AffineState& a, const AffineState& b) {
  AffineState out(a.n_ + b.n_);
  out.phase_ = (a.phase_ + b.phase_) % 8;
  out.r_ = a.r_ + b.r_;

  out.Q_.setZero(out.n_ + 1, out.n_ + 1);
  out.A_.setZero(out.n_, out.n_ + 1);
  out.b_.setZero(out.n_);
  out.pivots_.clear();
  out.pivots_.reserve(out.n_ + 1);

  if (a.r_ > 0) {
    out.Q_.block(0, 0, a.r_, a.r_) = a.Q_.block(0, 0, a.r_, a.r_);
  }
  if (b.r_ > 0) {
    out.Q_.block(a.r_, a.r_, b.r_, b.r_) = b.Q_.block(0, 0, b.r_, b.r_);
  }

  if (a.r_ > 0 && a.n_ > 0) {
    out.A_.block(0, 0, a.n_, a.r_) = a.A_.block(0, 0, a.n_, a.r_);
  }
  if (b.r_ > 0 && b.n_ > 0) {
    out.A_.block(a.n_, a.r_, b.n_, b.r_) = b.A_.block(0, 0, b.n_, b.r_);
  }

  if (a.n_ > 0)
    out.b_.segment(0, a.n_) = a.b_.segment(0, a.n_);
  if (b.n_ > 0)
    out.b_.segment(a.n_, b.n_) = b.b_.segment(0, b.n_);

  for (int c = 0; c < a.r_; ++c)
    out.pivots_.push_back(a.pivots_[c]);
  for (int c = 0; c < b.r_; ++c)
    out.pivots_.push_back(b.pivots_[c] + a.n_);
